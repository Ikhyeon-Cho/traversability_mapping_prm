...
private:
  ...
  elevation_msgs::OccupancyElevation elevationMap;  // this is received from mapping package. it is a 2d local map that
                                                    // includes height info
...
public:
  TraversabilityPRM() : nh("~"), planningFlag(false)
  {
    ...
    subElevationMap = nh.subscribe<elevation_msgs::OccupancyElevation>("/occupancy_map_local_height", 5,
                                                                       &TraversabilityPRM::elevationMapHandler, this);
  ...
  void updateMapBoundary()
  {
    ...
    map_max[0] = elevationMap.occupancy.info.origin.position.x +
                 elevationMap.occupancy.info.resolution * elevationMap.occupancy.info.width;
    map_max[1] = elevationMap.occupancy.info.origin.position.y +
                 elevationMap.occupancy.info.resolution * elevationMap.occupancy.info.height;
      ...
      if (elevationMap.occupancy.data[i] > 0)
      {
        for (int m = -inflationSize; m <= inflationSize; ++m)
        {
          for (int n = -inflationSize; n <= inflationSize; ++n)
          {
            int newIdX = idX + m;
            int newIdY = idY + n;
            if (newIdX < 0 || newIdX >= elevationMap.occupancy.info.width || newIdY < 0 ||
                newIdY >= elevationMap.occupancy.info.height)
              continue;
  ...
  void updateStatesAndEdges()
  {
    ...
    for (int i = 0; i < nearStates.size(); ++i)
    {
      for (int j = i + 1; j < nearStates.size(); ++j)
      {
        ...
        {  // edge is not connectable, delete old edge if it exists
          deleteEdge(nearStates[i], nearStates[j]);
        }
      }
    }
  ...
  void deleteEdge(state_t* stateA, state_t* stateB)
  {
    ...
    // and returns the iterator to the first element after that range. Then "erase" removes these elements (who's value
    // is unspecified).
    compareState = stateB;
  ...
  {
    if (rounded_x < 0 || rounded_x >= localMapArrayLength || rounded_y < 0 || rounded_y >= localMapArrayLength)
      return true;
  ...
  bool isIncollision(state_t* stateIn)
  {
    ...
    if (stateIn->x[0] <= map_min[0] || stateIn->x[0] >= map_max[0] || stateIn->x[1] <= map_min[1] ||
        stateIn->x[1] >= map_max[1])
      return true;
  ...
  void publishPRM()
  {
    ...
    if (pubSingleSourcePaths.getNumSubscribers() != 0)
    {
      ...
      markersPath.color.r = 0.3;
      markersPath.color.g = 0;
      markersPath.color.b = 1.0;
  ...
  void publishRoadmap2Cloud()
  {
    ...
    nodeCloud.resize(sizeCloud);
    pcl::PointCloud<PointType> adjacencyCloud;
    adjacencyCloud.resize(sizeCloud * sizeCloud);
